...
function parsePanelData(encodedData) {
    ...
    // Debug: Log the decoded buffer in hex and ASCII format
    ...
    const temperatureRange = (decoded[14] & 4) === 4 ? 'high' : 'low';
...
async function updateTemperatureRange(deviceId, setToHigh, override = false) {
    ...
    if (!currentPanelData.temperatureRange) {
        console.error('Temperature range not found in panel data');
        return;
    }
    const currentTempRangeState = currentPanelData.temperatureRange.toLowerCase();
...
function formatDecodedData(decoded) {
    console.log('Decoded Panel Data:');
    console.log('Index | Hexadecimal (Hex)                     | ASCII');
    console.log('------|---------------------------------------|----------------');

    for (let i = 0; i < decoded.length; i += 16) {
        const hexPart = Array.from(decoded.slice(i, i + 16))
            .map((byte) => byte.toString(16).padStart(2, '0')) // Convert to hex and pad with 0
            .join(' ');

        const asciiPart = Array.from(decoded.slice(i, i + 16))
            .map((byte) => (byte >= 32 && byte <= 126 ? String.fromCharCode(byte) : '.')) // Convert to ASCII or replace with '.'
            .join('');

        console.log(`${i.toString().padStart(4, '0')}  | ${hexPart.padEnd(39)} | ${asciiPart}`);
    }
}
